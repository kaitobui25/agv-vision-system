...
int main() {
...
#ifdef _WIN32
    // Initialize Winsock explicitly so WSAGetLastError() is valid and sockets work.
    WSADATA wsaData;
    ...
    while (running) {
        ...
        // Make sure the libmodbus context socket points to the listening socket
        // so modbus_tcp_accept() uses the proper listening socket instead of a
        // previously accepted (and possibly closed) client socket.
        modbus_set_socket(ctx, server_socket);

        // Accept connection using platform accept to avoid libmodbus replacing
        // the listening socket with the accepted client socket unexpectedly.
        while (running && client_socket == -1) {
            ...
            int accepted = accept(server_socket, (struct sockaddr*)&client_addr, &addrlen);
            if (accepted == -1) {
                int errnum = errno;
                ...
                printf("[hardware-sim] ERROR: Accept failed: errno=%d (%s)\n", errnum, strerror(errnum));
                client_socket = -1;
                break;
            }
            client_socket = accepted;
#endif
            // Tell libmodbus to use the accepted client socket
        ...
        // Ensure the accepted client socket is closed when the session ends so the
        // listening socket remains valid for subsequent accept calls.
        if (client_socket != -1) {
#ifdef _WIN32
            ...
            close(client_socket);
#endif
            // Reset libmodbus context socket back to listening socket to be safe
            modbus_set_socket(ctx, server_socket);
            client_socket = -1;
        }
